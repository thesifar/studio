...
import { ArrowRight, Sparkles, Play, Heart, Music, Loader2 } from "lucide-react";
...
export default function Home() {
  ...
  const featuredQuery = useMemoFirebase(() => {
    if (!db) return null;
    return query(collection(db, 'bhajans'), orderBy('createdAt', 'desc'), limit(4));
  }, [db]);

  const { data: featuredBhajans, isLoading } = useCollection(featuredQuery);
      ...
      <main className="flex-1">
        <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden">
          ...
          <div className="container relative z-10 px-4 text-center">
            <h1 className="font-headline text-5xl md:text-8xl font-bold text-white mb-8 tracking-tight leading-none">
              Experience the <span className="text-primary italic">Divine</span>
            </h1>
            
            <p className="text-lg md:text-2xl text-white/90 max-w-2xl mx-auto mb-12 font-body leading-relaxed">
