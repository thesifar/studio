...
export default function AdminBhajansPage() {
  ...
  const audioBhajans = filteredBhajans.filter(b => b.type === 'audio');
  const videoBhajans = filteredBhajans.filter(b => b.type === 'video');

  const BhajanTable = ({ data }: { data: typeof filteredBhajans }) => (
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                         ...
                         <DropdownMenuItem asChild>
                            <Link href={`/bhajans/${bhajan.id}`}>View Public</Link>
                         </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </div>
    ...
    <SidebarProvider>
      ...
      <SidebarInset className="bg-background">
        <header className="flex h-16 shrink-0 items-center justify-between px-6 border-b">
          ...
          <Button asChild className="bg-primary hover:bg-primary/90 rounded-full h-10 shadow-lg shadow-primary/20">
          ...
          <Tabs defaultValue="all" className="w-full">
            <div className="flex items-center justify-between mb-4">
              <TabsList className="bg-secondary/50 p-1 h-12 rounded-xl">
                <TabsTrigger value="all" className="rounded-lg px-6 h-10 data-[state=active]:bg-white data-[state=active]:shadow-sm">
                  All Items
                </TabsTrigger>
                <TabsTrigger value="audio" className="rounded-lg px-6 h-10 data-[state=active]:bg-white data-[state=active]:shadow-sm flex items-center gap-2">
                  <Music className="h-4 w-4" /> Audio
                </TabsTrigger>
                <TabsTrigger value="video" className="rounded-lg px-6 h-10 data-[state=active]:bg-white data-[state=active]:shadow-sm flex items-center gap-2">
                  <MonitorPlay className="h-4 w-4" /> Video
                </TabsTrigger>
              </TabsList>
              
              <div className="hidden sm:flex items-center gap-2 text-sm text-muted-foreground">
                <span className="font-bold text-primary">{filteredBhajans.length}</span> total entries
              </div>
            </div>

            <TabsContent value="all" className="mt-0 outline-none">
              <BhajanTable data={filteredBhajans} />
            </TabsContent>
            
            <TabsContent value="audio" className="mt-0 outline-none">
              <BhajanTable data={audioBhajans} />
            </TabsContent>
            
            <TabsContent value="video" className="mt-0 outline-none">
              <BhajanTable data={videoBhajans} />
            </TabsContent>
          </Tabs>
        </main>
      </SidebarInset>
